...
public static class Shapes
{
    ...
    public struct Job<S> : IJobFor where S : struct, IShape
    {
        ...
        public void Execute(int i)
        {
            ...
            positions[i] = transpose(MathExtensions.TransformVectors(positionTRS, p.positions));

            float3x4 n = transpose(MathExtensions.TransformVectors(normalTRS, p.normals, 0f));
        ...
        public static JobHandle ScheduleParallel(NativeArray<float3x4> positions, NativeArray<float3x4> normals, int resolution, float4x4 trs, JobHandle dependancy)
        {
            return new Job<S>
            {
                ...
                positionTRS = MathExtensions.Get3x4(trs),
                normalTRS = MathExtensions.Get3x4(transpose(inverse(trs)))
