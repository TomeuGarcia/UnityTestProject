...
namespace ProceduralMeshes.Generators
{
	...
	public struct PointyHexagonGrid : IMeshGenerator
	{
		...
		public Bounds Bounds => new Bounds(Vector3.zero, 
										   new Vector3(Resolution > 1 ? 0.5f + 0.25f / Resolution : 0.5f * sqrt(3.0f), 0.0f, 0.75f + 0.25f / Resolution));
		...
		public void Execute<S>(int z, S streams) where S : struct, IMeshStreams
		{
			...
			if (Resolution > 1)
            {
				centerOffset.x = (((z & 1) == 1 ? 1.5f : 0.5f) - Resolution) * h;
			...
			{
				float2 center = (float2(2.0f * h * x, 0.75f * z) + centerOffset) / Resolution;
